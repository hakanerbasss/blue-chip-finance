...
class HomeFragment : Fragment() {
        ...
        scope.launch {
            try {
                ...
                if (isAdded && activity != null) {
                    Toast.makeText(requireActivity().applicationContext, "Fiyatlar yüklenemedi", Toast.LENGTH_SHORT).show()
                }
            } finally {
    ...
    private fun openUrl(url: String) {
        try {
            ...
            if (isAdded && activity != null) {
                Toast.makeText(requireActivity().applicationContext, "Tarayıcı açılamadı", Toast.LENGTH_SHORT).show()
            }
        }
    }
